...
#define DEFAULT_INIT_DEVIATION		0.3f
#define DEFAULT_MATCH_ROUNDS		4
#define DEFAULT_GAMES_PER_MATCH		2
...
private:
	...
	static NeuralNetwork** generate_inherited_NN(NeuralNetwork* parent, unsigned amount, float deviation);
	...
	static void add_random_noise(NeuralNetwork* nn, float deviation);
	...
	static void play_survival_tournament(NeuralNetwork** NNs, float* scores, unsigned n_players, unsigned match_rounds, unsigned games_per_match);
	...
	static void deeper_search_tournament(NeuralNetwork* default_scheduler, NeuralNetwork** NNs, float* scores, unsigned n_players, unsigned games);
...
public:
	...
	static unsigned char NNvsNN(float sec_per_move, Board initial_board, NeuralNetwork* nn_1, NeuralNetwork* nn_2, bool print_boards = false, bool clear_console = true);
	...
	static void training_session(const char* last_session_storage, const char* this_session_storage, unsigned last_survival_size, unsigned this_session_size, unsigned survival_ranking, float deviation, Training_Type type, bool default_heuristic);
};